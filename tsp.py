...
import didppy as dp
from utils.read_tsp import validate
from utils.read_tsptw import read_xycoords
...
def create_DPmodel_kuroiwa(n, nodes, edges, non_zero_base_case, target=None):
    if target is None:
...
def create_DPmodel_cappart(n, nodes, edges):
    model = dp.Model()
...
def create_solver(
    ...
    def create_g_evaluators(scaling_factor):
        ...
        for dest in customers:
            g_evaluators = {
                    f"visit {dest}": create_g_function(
                        location, edges, dest, 
...
if __name__ == "__main__":
    ...
    # Read TSPTW instance (TSP instances are equivalent to TSPTW instances ignoring the time windows)
    ...
    if cost is not None and validate(n, edges, tour, cost):
