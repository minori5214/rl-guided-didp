...
def read_xycoords(filename):
    ...
    return n, nodes, edges, a, b, x_coord, y_coord
...
def read_rl(filename):
    ...
    return n, travel_time, x_coord, y_coord, time_windows
...
def validate(n, edges, a, b, solution, cost, tolerance=1e-4, verbose=1):
    previous = solution[0]
    if previous != 0:
        if verbose: print(
    ...
    for i in solution[1:-1]:
        ...
        if time > b[i]:
            if verbose: print("The time {} exceeds the deadline {} for {}".format(time, b[i], i))
            return False
