...
use_tsp_nn_model=1 # 1 if TSP RL model is used
...
command="python3 tsptw.py --n-city ${n_city} --train-n-city ${train_n_city} \
                    --train-max-tw-gap ${train_max_tw_gap} --train-max-tw-size ${train_max_tw_size} \
                    ...
                    --use-tsp-nn-model ${use_tsp_nn_model} \
                    --softmax-temperature ${softmax_temperature} \
                    --no-policy-accumulation ${no_policy_accumulation} \
                    --file ${i}.txt"

python3 wrapper.py --command ${command} --memory-limit ${memory_limit}
